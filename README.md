...
in `data/reading-profile.md` and maintains two top 20 lists: **non-fiction**
and **other books**.
...
## Build the index (one time, ~7 GB download)
...
computes both lists once and exits (venv from the index build step):
...
- **Current reading wish:** bullet points describing what you want to read
  right now. This is the only part that uses the LLM. Leave it empty for
  pure taste-based recommendations (no GPU involved).
- **Books read:** a table with title, author and type (`nonfiction` or
  `other`) of books you read and enjoyed. They form your taste profile, are
  never proposed again, and disappear from the lists. English original
  titles match the catalog best — check
  [data/profile-match.md](data/profile-match.md) to see which entries were
  recognized and fix the spelling of the rest.

## Watching

- **Recommendations:** [data/top_nonfiction.md](data/top_nonfiction.md) and
  [data/top_other.md](data/top_other.md). Score detail per book:
  T = taste closeness (CF), Q = Goodreads reader quality, W = wish fit (LLM).
  "Because you liked" names the favourites that pulled the book in.
...
| `MODEL` | `gemma3:12b` | Ollama model for the re-ranker |
